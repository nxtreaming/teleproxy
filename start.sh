...
# Set default values
PORT=${PORT:-443}
# Externally-advertised port for the connection link.
# Set to the host-side port when Docker maps a different external port,
# e.g. `-p 4443:443` → EXTERNAL_PORT=4443.
...
TOML_CONFIG="data/config.toml"
{
    ...
    echo ""
    # Write secrets from comma-separated SECRET (supports hex:label syntax)
    if [ -n "$SECRET" ]; then
        _save_ifs2="$IFS"
        IFS=','
        for _cs in $SECRET; do
            ...
            if [ -n "$_cs" ]; then
                _chex=$(printf '%s' "$_cs" | cut -d: -f1)
                _clbl=$(printf '%s' "$_cs" | cut -d: -f2 -s)
                [ -n "$_chex" ] || continue
                echo "[[secret]]"
                echo "key = \"$_chex\""
                [ -n "$_clbl" ] && echo "label = \"$_clbl\""
                echo ""
            fi
        done
        IFS="$_save_ifs2"
...
for _s in "$@"; do
    ...
    ./teleproxy link --server "$_host" --port "$EXTERNAL_PORT" --secret "$_full" $_label_arg
done
